...
if otlp.ENABLE:
    resource = Resource.create(
        attributes={"service.name": otlp.OTEL_RESOURCE_ATTRIBUTES}
    )
    ...
    span_processor = BatchSpanProcessor(
        OTLPSpanExporter(
            endpoint=otlp.OTEL_EXPORTER_OTLP_ENDPOINT, insecure=otlp.INSECURE_ENDPOINT
        )
    )
    ...
    application = OpenTelemetryMiddleware(
        application, None, None, trace.get_tracer_provider()
    )
